...
class PostPage(BlogHandler):
    def get(self, post_id):
        ...
        post = db.get(key)
        comments = db.GqlQuery("SELECT * FROM Comment where commentID =:1 \n"
                               "ORDER BY created ASC",  int(post.key().id()))
    ...
    def post(self):
        ...
        if subject and content:
            p = Post(parent=blog_key(), subject=subject, content=content,
                     author=str(self.user.name), likes=0)
            p.put()
...
class DeleteComment(BlogHandler):
    ...
    def post(self, comment_id):
        ...
        if answer == "yes":
            ...
            # redirect back to post page
            return self.redirect('/blog/%s' % c.commentID)


# regular expressions/functions to check for valid user names/passwords/emails
# on signup
